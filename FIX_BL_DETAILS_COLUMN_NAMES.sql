-- Fix get_bl_with_details function with correct column names
-- The detail_bl table uses lowercase 'tva', not 'TVA'
...
BEGIN
    EXECUTE format('
        SELECT json_build_object(
            ...
            ''details'', COALESCE(
                (
                    ...
                    FROM "%1$s".detail_bl d
                    LEFT JOIN "%1$s".article a ON a."Narticle" = d."Narticle"
        ...
        FROM "%1$s".bl bl
        LEFT JOIN "%1$s".client c ON c."Nclient" = bl."Nclient"
        WHERE bl."NFact" = $1
    ', p_tenant)
    INTO v_result
    USING p_nfact;
