...
-- Script PostgreSQL LOCAL (sans rôles Supabase)
-- =====================================================
...
CREATE OR REPLACE FUNCTION public.insert_avoir(
  p_tenant TEXT, p_nclient TEXT, p_date_avoir DATE, p_document_type TEXT,
  p_document_ref INTEGER, p_montant_ht NUMERIC, p_tva NUMERIC,
  p_montant_ttc NUMERIC, p_motif TEXT DEFAULT NULL
...
CREATE OR REPLACE FUNCTION public.insert_detail_avoir(
  p_tenant TEXT, p_avoir_id INTEGER, p_narticle TEXT,
  ...
  FROM (
    SELECT av.id, av.tenant, av.nclient, av.date_avoir, av.document_type,
      ...
      (SELECT json_agg(row_to_json(d))
       FROM (SELECT da.* FROM public.detail_avoir da WHERE da.avoir_id = av.id) d
      ) AS details
