-- =====================================================
-- FONCTIONS RPC FINALES CORRIGÉES pour les factures
-- À exécuter dans l'éditeur SQL de Supabase
-- =====================================================

-- 1. Fonction corrigée pour la liste des factures (sans GROUP BY problématique)
...
BEGIN
  ...
  -- Récupérer les factures (requête simplifiée sans GROUP BY)
  EXECUTE format('
    SELECT json_agg(
      json_build_object(
        ''nfact'', nfact,
...
-- 3. Fonction pour le PDF avec toutes les données nécessaires
...
BEGIN
  ...
  -- Récupérer toutes les données pour le PDF
  EXECUTE format('
    SELECT json_build_object(
      ''nfact'', f.nfact,
      ''nclient'', f.nclient,
...
-- Tests pour vérifier que tout fonctionne maintenant
