-- CRÉATION PROPRE DES FONCTIONS CLIENTS - À EXÉCUTER APRÈS LE NETTOYAGE
-- Copiez ce code et exécutez-le dans Supabase SQL Editor APRÈS cleanup-all-client-functions.sql

-- IMPORTANT: Vérifiez d'abord qu'aucune fonction client n'existe plus avec cette requête:
-- SELECT proname FROM pg_proc WHERE proname LIKE '%client%' AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public');
...
-- 3. Vérifier que les fonctions sont créées correctement
SELECT 
    proname as function_name,
    pg_get_function_identity_arguments(oid) as arguments,
    'SUCCESS: Function created' as status
FROM pg_proc 
WHERE proname IN ('get_clients_by_tenant', 'insert_client_to_tenant')
