-- Fonction RPC pour générer un rapport des ventes avec calcul de marge
-- Cette fonction combine les BL et factures avec calcul automatique de la marge
...
DECLARE
  ...
  final_result JSON;
BEGIN
  -- Construire la requête pour récupérer les ventes avec marge
  WITH combined_sales AS (
    ...
    SELECT 
      ...
      COALESCE(bl.montant_ttc, bl.montant_ht + bl.tva, 0) as montant_ttc,
      -- Calcul de la marge pour BL
      COALESCE(
        (SELECT SUM(dbl.qte * (dbl.prix - COALESCE(a.prix_unitaire, 0)))
         FROM %I.detail_bl dbl
         LEFT JOIN %I.articles a ON a.narticle = dbl.narticle
         WHERE dbl.nbl = bl.nbl), 0
      ) as marge,
    ...
    WHERE bl.date_fact BETWEEN p_date_from AND p_date_to
      AND (p_type_filter = 'ALL' OR p_type_filter = 'BL')
      AND (p_client_code IS NULL OR bl.nclient ILIKE '%%' || p_client_code || '%%')
    ...
    SELECT 
      ...
      f.created_at
    FROM %I.factures f
    LEFT JOIN %I.clients c ON c.nclient = f.nclient
    WHERE f.date_fact BETWEEN p_date_from AND p_date_to
      AND (p_type_filter = 'ALL' OR p_type_filter = 'FACTURE')
      AND (p_client_code IS NULL OR f.nclient ILIKE '%%' || p_client_code || '%%')
  ),
...
EXCEPTION
  WHEN OTHERS THEN
    ...
    RETURN '{"sales":[],"totals":{"count_bl":0,"count_factures":0,"total_count":0,"total_ht":0,"total_tva":0,"total_ttc":0,"total_marge":0,"marge_percentage_avg":0},"error":"' || SQLERRM || '"}'::json;
END;
$$;
...
-- SELECT get_sales_report_with_margin('2025_bu01', '2025-12-21', '2025-12-21', 'BL', 'CL01');