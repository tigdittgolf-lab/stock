#!/usr/bin/env node

// Script professionnel pour corriger automatiquement l'endpoint BL
import fs from 'fs';
import path from 'path';
import { fileURLToPath } from 'url';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

const filePath = path.join(__dirname, 'src', 'routes', 'sales-clean.ts');

console.log('🔧 Fixing BL endpoint automatically...');

try {
  // Lire le fichier
  let content = fs.readFileSync(filePath, 'utf8');
  
  // Pattern pour trouver l'endpoint BL complet
  const blEndpointPattern = /\/\/ GET \/api\/sales\/delivery-notes - Récupérer la liste des bons de livraison\s*sales\.get\('\/delivery-notes', async \(c\) => \{[\s\S]*?\}\);/;
  
  // Nouveau code professionnel
  const newBlEndpoint = `// GET /api/sales/delivery-notes - Récupérer la liste des bons de livraison
sales.get('/delivery-notes', async (c) => {
  try {
    const tenant = c.get('tenant');
    if (!tenant) {
      return c.json({ success: false, error: 'Tenant header required' }, 400);
    }

    console.log(\`📋 Fetching REAL delivery notes for tenant: \${tenant}\`);

    // Utiliser la fonction RPC pour récupérer les VRAIS BL
    const { data: blData, error: blError } = await supabaseAdmin.rpc('get_bl_list_by_tenant', {
      p_tenant: tenant
    });

    if (blError) {
      console.error('❌ Failed to fetch REAL BL data:', blError);
      return c.json({
        success: true,
        data: [],
        message: 'Failed to fetch REAL BL data - RPC error',
        tenant: tenant,
        source: 'fallback'
      });
    }

    // Transformer les données pour correspondre au format attendu
    const enrichedBL = (blData || []).map(bl => ({
      id: bl.nfact,
      nbl: bl.nfact,
      nclient: bl.nclient,
      client_name: bl.client_name || bl.nclient,
      date_fact: bl.date_fact,
      montant_ht: parseFloat(bl.montant_ht || '0'),
      tva: parseFloat(bl.tva || '0'),
      montant_ttc: parseFloat(bl.montant_ht || '0') + parseFloat(bl.tva || '0'),
      created_at: bl.created_at,
      type: 'bl'
    }));

    console.log(\`✅ Found \${enrichedBL.length} REAL delivery notes from database via RPC\`);

    return c.json({
      success: true,
      data: enrichedBL,
      tenant: tenant,
      source: 'real_database_via_rpc'
    });

  } catch (error) {
    console.error('❌ Error fetching REAL delivery notes:', error);
    return c.json({ 
      success: false, 
      error: 'Erreur lors de la récupération des bons de livraison'
    }, 500);
  }
});`;

  // Vérifier si le pattern existe
  if (blEndpointPattern.test(content)) {
    // Remplacer l'ancien endpoint par le nouveau
    content = content.replace(blEndpointPattern, newBlEndpoint);
    
    // Écrire le fichier corrigé
    fs.writeFileSync(filePath, content, 'utf8');
    
    console.log('✅ BL endpoint fixed successfully!');
    console.log('🔄 Please restart the backend server');
    
  } else {
    console.error('❌ Could not find BL endpoint pattern in file');
    console.log('🔍 Trying alternative approach...');
    
    // Approche alternative : chercher juste le début de la fonction
    const simplePattern = /sales\.get\('\/delivery-notes', async \(c\) => \{[\s\S]*?\}\);/;
    
    if (simplePattern.test(content)) {
      content = content.replace(simplePattern, newBlEndpoint.split('\n').slice(1).join('\n'));
      fs.writeFileSync(filePath, content, 'utf8');
      console.log('✅ BL endpoint fixed with alternative approach!');
    } else {
      console.error('❌ Could not find any BL endpoint pattern');
      process.exit(1);
    }
  }
  
} catch (error) {
  console.error('❌ Error fixing BL endpoint:', error.message);
  process.exit(1);
}