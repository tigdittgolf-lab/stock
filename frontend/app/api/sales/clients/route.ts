...
export async function GET(request: NextRequest) {
  ...
  if (BACKEND_URL) {
    try {
      ...
      console.warn(`[clients] Backend responded ${res.status}, falling back to Supabase direct`);
    } catch (e) {
  ...
  try {
    ...
    const rows = await readTable(tenant, 'client');
    ...
    if (msg.includes('does not exist') || msg.includes('n\'existe pas') || msg.includes('HTTP 404') || msg.includes('HTTP 400') || msg.includes('HTTP 422')) {
      console.warn(`⚠️ [clients] Schéma ${tenant} introuvable dans Supabase, retour tableau vide`);
