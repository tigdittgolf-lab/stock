...
export async function GET(
  ...
  try {
    ...
    console.log(`🔄 Frontend PDF API: Forwarding delivery note PDF request for ID ${id}, tenant ${tenant}`);
    ...
    if (contentType?.includes('application/pdf')) {
      ...
      console.log(`✅ Frontend PDF API: Successfully forwarded PDF for delivery note ${id}`);
      
      return new NextResponse(pdfBuffer, {
        status: 200,
        headers: {
          'Content-Type': 'application/pdf',
          'Content-Disposition': `attachment; filename="bon_livraison_${id}.pdf"`
        }
      });
    } else {
