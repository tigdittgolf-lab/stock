...
export async function GET(
  ...
  try {
    ...
    console.log(`🔄 Frontend PDF API: Forwarding small delivery note PDF request for ID ${id}, tenant ${tenant}`);
    
    // Utiliser Tailscale tunnel pour accéder au backend local
    const backendUrl = 'https://desktop-bhhs068.tail1d9c54.ts.net/api/pdf/delivery-note-small/' + id;
    ...
    if (contentType?.includes('application/pdf')) {
      ...
      console.log(`✅ Frontend PDF API: Successfully forwarded small PDF for delivery note ${id}`);
      
      return new NextResponse(pdfBuffer, {
        status: 200,
        headers: {
          'Content-Type': 'application/pdf',
          'Content-Disposition': `attachment; filename="bon_livraison_small_${id}.pdf"`
        }
      });
    } else {
