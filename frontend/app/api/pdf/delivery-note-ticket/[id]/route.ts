...
export async function GET(
  ...
  try {
    ...
    if (!response.ok) {
      console.warn(`[pdf/delivery-note-ticket] Backend ${response.status}, redirect to print page`);
      return NextResponse.redirect(new URL(`/print/bl/${validId}?tenant=${tenant}&size=ticket`, request.url));
    }
    ...
    console.warn(`[pdf/delivery-note-ticket] Backend inaccessible, redirect to /print/bl/${validId}`);
    return NextResponse.redirect(new URL(`/print/bl/${validId}?tenant=${tenant}&size=ticket`, request.url));
  }
}
