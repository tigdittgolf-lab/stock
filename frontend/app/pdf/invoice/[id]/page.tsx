...
export default function InvoicePDFPage() {
  ...
  const generate = async () => {
    ...
    try {
      const [factRes, companyRes] = await Promise.all([
        fetch(`/api/sales/invoices?id=${id}`, {
      ...
      if (!factJson.success) { setStatus(`❌ Facture ${id} introuvable: ${factJson.error}`); return; }
      factData = factJson.data;
      ...
      if (!factData.client_name && factData.nclient) {
        try {
          for (const col of ['Nclient', 'nclient']) {
            const r = await fetch(
              `${SUPA_URL}/rest/v1/client?${col}=eq.${factData.nclient}&select=Raison_sociale,raison_sociale,nom&limit=1`,
              { headers: supaHeaders(tenant) }
            );
            if (r.ok) {
              const rows = await r.json();
              if (rows?.[0]) {
                factData.client_name = rows[0].Raison_sociale || rows[0].raison_sociale || rows[0].nom || '';
                if (factData.client_name) break;
              }
            }
          }
        } catch { /* non critique */ }
      ...
      const details: any[] = factData.detail_fact || factData.details || [];
    ...
    const fact = {
      nfact: factData.nfact || parseInt(id),
      date_fact: factData.date_fact || factData.date || '',
      client: {
        raison_sociale: factData.client_name || factData.raison_sociale || String(factData.nclient || ''),
        adresse: factData.adresse_client || '',
      },
      montant_ht: parseFloat(factData.montant_ht?.toString() || '0') || 0,
      tva: parseFloat(factData.tva?.toString() || '0') || 0,
      montant_ttc: parseFloat((factData.montant_ttc || factData.total_ttc)?.toString() || '0') || 0,
      timbre: parseFloat(factData.timbre?.toString() || '0') || 0,
      details: (factData.detail_fact || factData.details || []).map((d: any) => ({
    ...
    if (!fact.montant_ttc) fact.montant_ttc = fact.montant_ht + fact.tva + fact.timbre;
    ...
    try {
      ...
      doc.text('FACTURE', 105, y, { align: 'center' }); y += 10;
      doc.setLineWidth(0.5); doc.line(20, y, 190, y); y += 15;

      // Droite: N° + date + client
      let ry = y;
      doc.setFontSize(10); doc.setFont('helvetica', 'normal');
      doc.text(`Facture N: ${fact.nfact}`, 140, ry); ry += 5;
      doc.text(`Date: ${fmtDate(fact.date_fact)}`, 140, ry); ry += 8;
      doc.setFont('helvetica', 'bold'); doc.text('Client:', 140, ry); ry += 5;
      doc.setFont('helvetica', 'normal');
      doc.text(fact.client.raison_sociale.substring(0, 25), 140, ry); ry += 5;
      if (fact.client.adresse) { doc.text(fact.client.adresse.substring(0, 25), 140, ry); ry += 5; }
      ...
      y += 8;
      doc.setFontSize(10); doc.setFont('helvetica', 'normal');
      doc.text('Sous-total HT:', 120, y); doc.text(fmt(fact.montant_ht), 190, y, { align: 'right' }); y += 6;
      doc.text('TVA:', 120, y); doc.text(fmt(fact.tva), 190, y, { align: 'right' }); y += 6;
      if (fact.timbre > 0) { doc.text('Timbre:', 120, y); doc.text(fmt(fact.timbre), 190, y, { align: 'right' }); y += 6; }
      y += 2;
      doc.setFont('helvetica', 'bold'); doc.setFontSize(12);
      doc.text('TOTAL TTC:', 120, y); doc.text(fmt(fact.montant_ttc), 190, y, { align: 'right' });

      // Montant en lettres
      y += 15;
      doc.setFontSize(9); doc.setFont('helvetica', 'normal');
      doc.line(20, y - 5, 190, y - 5);
      doc.text('Arrete la presente facture a la somme de :', 20, y); y += 12;
      const words = numberToWordsFr(fact.montant_ttc);
